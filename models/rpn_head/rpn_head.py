...
class RPNHead(nn.Module):
    ...
    def __init__(self, strides):
        ...
        for ri, r in enumerate(self.anchor_ratio):
            for si, s in enumerate(self.anchor_scale):
                ...
                w = s/sqrt_r
                h = s*sqrt_r
                self.anchor_template[i, :] = torch.Tensor([self.anchor_stride[0]*0.5, self.anchor_stride[0]*0.5, w, h])
    ...
    def forward(self, feature_maps, img_meta, gt_bboxes=None):
        ...
        # generate regression results, bbox proposals
    ...
    def generate_anchors(self, obj_reg_scores, img_meta):
        ...
        Args:
            obj_reg_scores: size=(B, H, W, anchor_ratio * anchor_scale, 4)
        ...
        anchor_center = torch.stack((anchor_center[1], anchor_center[0]), dim=-1)*self.anchor_stride[0]
        anchors[..., [0, 1]] += anchor_center[None, ..., None, :].type(anchors.dtype).to(anchors.device)

        if self.training: # ignore outliers
            for b, im_size in enumerate(img_meta['img_size'].to(anchors.device)):
                anchors_corner = xywh2xyxy(anchors[b])
    ...
    def loss(self, obj_cls_score, obj_reg_score, anchor, gt_bbox, assign_results, pos_ind, neg_ind):
        ...
        reg_losses = SmoothL1Loss(pos_reg_score, gt_score, weights=1.0 / sam_ind.size(0), sigma=3.0, reduction='sum')
